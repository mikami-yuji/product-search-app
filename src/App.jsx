...
const ProductImage = ({ dirHandle, filename, productCode, productType, materialName, webImages, className, onClick }) => {
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      ...
      // 4. Dynamic URL for 既製品 (multiple candidates based on material name)
      if (productCode && productType && productType === '既製品') {
        const codeStr = String(productCode);
        if (codeStr.length >= 5) {
          ...
          if (productType === '既製品') {
            // Check if material name contains specific keywords
...
const ProductDetailsModal = ({ product, onClose, dirHandle, webImages }) => {
  ...
  useEffect(() => {
    const checkImages = async () => {
      ...
      // Check web image
      if (webImages && product['商品コード']) {
        const codeStr = String(product['商品コード']);
        if (webImages[codeStr]) {
          images.push({ url: webImages[codeStr], suffix: 'Web', source: 'web' });
        }
      }

      // Check dynamic URL for 既製品
      const productType = product['種別'] || product['形状'];
      const materialName = product['材質名称'];
      if (product['商品コード'] && productType && productType === '既製品') {
        const codeStr = String(product['商品コード']);
        if (codeStr.length >= 5) {
          const codePart = codeStr.slice(2, 5);
          let candidates = [];
